...
export const ShoppingList = ({ modalOpen, modalClose }) => {
  ...
  const handleFormSubmit = async e => {
    ...
    sendMessage(`Нова заявка! Хочу Костюм!!!
      \nТовар: ${products.map(
        product => `${product.type};
        Розмір: ${product.size};
        Колір: ${product.color};
        Колір сорочки: ${product.shirtColor};
        Колір штанів: ${product.pantsColor};
        ...
        <p className="modalLinkText">на головну</p>
          ...
          {products && products.map(product => (
            <div key={product.id} className="shoppingList-product">
              <p className="shoppingList-productName">Костюм "{product.type}"</p>
              {product.color && (   <p className="shoppingList-productName">Колір: {product.color || 'Не вибрано'}</p>)}
              {(product.shirtColor || product.pantsColor) &&  (
                <>
                  <p className="shoppingList-productName">
                    Колір сорочки: {product.shirtColor || 'Не вибрано'}
                  </p>
                  <p  className="shoppingList-productName"> 
                    Колір штанів: {product.pantsColor || 'Не вибрано'}</p>
                </>
              )}
